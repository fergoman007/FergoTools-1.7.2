...
public class CoalFurnace
{
    public static final class BlockCoalFurnace extends BlockFurnaceFT
    {
        ...
        public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
            if (!worldIn.isRemote && !playerIn.isSneaking())
            {
                ...
                if (furnace != null)
                {
                    openGui(FergoTools.instance, GuiIds.coalFurnace.ordinal(), worldIn, pos, playerIn);
        ...
        public static void setState(boolean active, World world, BlockPos pos)
        {
            ...
            if (active)
            {
                world.setBlockState(pos, BlockStateHelper.getStateWithProps(ModBlocks.coalFurnaceActive, FACING, state.getValue(FACING)), 3);
                world.setBlockState(pos, BlockStateHelper.getStateWithProps(ModBlocks.coalFurnaceActive, FACING, state.getValue(FACING)), 3);
            }
            else
            {
                world.setBlockState(pos, BlockStateHelper.getStateWithProps(ModBlocks.coalFurnaceIdle, FACING, state.getValue(FACING)), 3);
                world.setBlockState(pos, BlockStateHelper.getStateWithProps(ModBlocks.coalFurnaceIdle, FACING, state.getValue(FACING)), 3);
            }
    ...
    public static final class TileCoalFurnace extends TileFurnaceFergo
    {
        @Override
        public String getCommandSenderName() {
            return this.hasCustomName() ? this.customName : Locale.containerCoalFurnace;
        }
        ...
        public int getFurnaceSpeed(ItemStack stack) {
            return FurnaceConstants.Speed.coalFurnace;
        }
    ...
    public static final class ContainerCoalFurnace extends ContainerFurnaceFergo
    {
        ...
        public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int slotIndex){
            ...
            if (slot != null && slot.getHasStack()){
                ...
                if (slotIndex == 2){
                    ...
                    }else if (TileCoalFurnace.isItemFuel(itemstack1)){
    ...
    public static final class GuiCoalFurnace extends GuiFurnaceFergo
    {
        public GuiCoalFurnace(InventoryPlayer invPlayer, IInventory furnace)
        {
            super(new ContainerCoalFurnace(invPlayer, furnace), invPlayer, furnace);
        }
        ...
        public void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
        {
            ...
            GuiHelper.bindTexture(Textures.coalFurnaceGui);
            int startX = (width - xSize) / 2;
            int startY = (height - ySize) / 2;
            drawTexturedModalRect(startX, startY, 0, 0, xSize, ySize);

            if (TileCoalFurnace.isBurning(this.furnace))
            {
