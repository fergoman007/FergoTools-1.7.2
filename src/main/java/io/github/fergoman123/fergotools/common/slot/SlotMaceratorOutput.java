...
public class SlotMaceratorOutput extends Slot
{
    ...
    public SlotMaceratorOutput(EntityPlayer player, IInventory p_i45793_2_, int slotIndex, int xPosition, int yPosition)
    {
    ...
    protected void onCrafting(ItemStack stack)
    {
        ...
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreCoal);
        }

        if (stack.getItem() == Items.iron_ingot)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreIron);
        }

        if (stack.getItem() == Items.gold_ingot)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreGold);
        }

        if (stack.getItem() == Items.diamond)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreDiamond);
        }

        if (stack.getItem() == Items.emerald)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreGemEmerald);
        }

        if (stack.getItem() == new ItemStack(Items.dye, 1, 4).getItem())
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreLapis);
        }

        if (stack.getItem() == ModItems.shardExp)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreExperience);
        }

        if (stack.getItem() == ModItems.ingotObsidian)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreObsidian);
        }
        ...
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreGemLapis);
        }

        if (stack.getItem() == ModItems.ingotBronze)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreBronze);
        }

        if (stack.getItem() == ModItems.ingotAdamantium)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreAdamantium);
        }

        if (stack.getItem() == ModItems.gemRedstone)
        {
            this.thePlayer.triggerAchievement(ModAchievements.oreGemRedstone);
        }
    }
}
